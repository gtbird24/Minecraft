...
public class GuiChat extends GuiScreen
{
    ...
    //Hack - Do Console Command Entered
    public void doConsoleCommand(String s)
    {
    	if(s.startsWith(".up"))
    	{
	    	int units = 100;
	    	try
	    	{
	    		units = Integer.parseInt(s.substring(3));
	    	}catch (Exception ex)
	    	{
	    		this.mc.ingameGUI.drawString(this.mc.fontRenderer, "Exception Caught", 12, 12, 0x0000ff);
	    	}
	    	this.mc.ingameGUI.drawString(this.mc.fontRenderer, "Moving UP " + units, 8, 8, 0x0000ff);
	    	this.mc.thePlayer.setPosition(this.mc.thePlayer.getPosition(1.0F).xCoord, this.mc.thePlayer.getPosition(1.0F).yCoord + units, this.mc.thePlayer.getPosition(1.0F).zCoord);
    	}
    ...
    protected void keyTyped(char par1, int par2)
    {
        ...
        else if (par2 == 28) //28 = Enter Key
        {
            String var3 = this.inputField.getText().trim();

            if(var3.length() > 0 && var3.startsWith("."))
            {
            	//Draw what you typed onto screen
                //This is where we could use commands to do stuff
            	doConsoleCommand(var3);
                //this.mc.ingameGUI.drawString(this.mc.fontRenderer, "You Typed" + var3, 2, 2, 0x0000ff);
            }            
